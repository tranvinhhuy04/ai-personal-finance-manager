...
export async function connectDB(): Promise<void> {
  ...
  if (!mongoUri) {
    throw new Error(
      '[Transaction] Database URI is not defined. Set MONGO_URI or MONGODB_URI in .env.',
    );
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`✓ MongoDB connected (Transaction Service) – attempt ${attempt}/${MAX_RETRIES}`);
      return;
    } catch (error: unknown) {
