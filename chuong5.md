...
graph TD
    ...
    K --> M["build_query_plan<br/>target_service, fields"]
    L --> M
    M --> N["build_rule_based_answer<br/>template + context"]
    
    N --> O{use_llm?}
    O -->|YES| P["Gemini generate<br/>financial_answer<br/>temp=0.2<br/>maxTokens=400"]
    O -->|NO| Q["return rule-based"]
    
    P --> Q
    Q --> R["Response kèm<br/>intent, confidence, scores"]
    
    R -->|return| S["🔵 API Gateway BFF"]
    S --> T["Append usage log<br/>Identity Service"]
    T --> U["✅ Mobile App<br/>Display answer"]
...
- **[3] AI Service — NLPService.answer_question()**:
  ...
  - **[3e]** `build_query_plan(intent)`: Sinh kế hoạch truy vấn (target_service, fields needed)
  - **[3f]** `build_rule_based_answer(intent, context)`: Sinh câu trả lời từ template
  - **[3g]** Nếu `use_llm=true` và có Gemini key → `generate_financial_answer(question, intent, context)` với `temp=0.2`, `maxTokens=400`
...
- **[1] API Gateway (aiAdvisorBff.ts)**: Nhận message, kiểm tra cache Redis (key = `SHA256(userId+sessionId+message)`)
  ...
  - Cache miss → proceed to AI Service

- **[2] POST http://ai-service:8000/api/v1/ai/advisor/chat**: Body chứa `{message, user_id, session_id, risk_profile, financial_profile, gemini_api_keys, model}`

- **[3] AdvisorOrchestrator.handle(request)**:
  - **[3a]** **Route Classifier** (regex pattern, NHANH):
    - `INTERNAL_DATA_PATTERN`: "chi tiêu|thu nhập|tiết kiệm|số dư|giao dịch" → `route = "internal_data"`
    - `EXTERNAL_FINANCIAL_PATTERN`: "giá vàng|tỷ giá|chứng khoán|lãi suất|bitcoin" → `route = "external_financial_data"`
    - `OUT_OF_SCOPE_PATTERN`: "thời tiết|nấu ăn|bóng đá" → `route = "out_of_scope"` → reject ngay
  
  - **[3b]** **Intent Extraction LLM** (`temperature=0`, `maxTokens=128`): Phân loại `transaction_lookup | financial_advice | chart_analysis | general_knowledge` → JSON: `{intent, confidence, time_range, category, amount}`
  
  - **[3c]** **RetrievalLayer.fetch_structured_data()** (tùy intent):
    - `transaction_lookup / chart_analysis`: Motor aggregate MongoDB transactions (filter `userId + time_range`)
    - `external_financial_data`: HTTP calls song song → `gold-api.com` (giá vàng), `open.er-api.com` (tỷ giá), `24h.com.vn` (giá vàng SJC)
  
  - **[3d]** **FinancialMath** (tính toán thuần Python): `compute_savings_rate()`, `compute_roi()`, v.v.
  
  - **[3e]** **MemoryStore**: Lấy 6 lượt hội thoại gần nhất của session
  
  - **[3f]** **build_advisor_system_prompt()**: Xây dựng prompt kèm `financial_profile`, `risk_profile`, calculations, tool_context
  
  - **[3g]** **call_gemini_with_rotation()**: Auto-rotation qua API Key Pool, retry khi 429
  
  - **[3h]** **Gemini 2.5 Flash**: `temperature=0.3`, Google Search Grounding → raw answer
  
  - **[3i]** **apply_output_guardrails()**: 
    - Kiểm tra `FORBIDDEN_PATTERNS` ("lợi nhuận chắc chắn", "all-in", "đòn bẩy cao")
    - Sanitize PII (email, điện thoại, tài khoản)
    - Nếu vi phạm → `blocked=True`, thay bằng fallback msg
  
  - **[3j]** **MemoryStore.append()**: Lưu lượt hội thoại vào session memory
  
  - **[3k]** **Redis cache 120s**: Persist kết quả cuối

- **[4] Response**: Trả về client kèm `{answer, route, intent, blocked, exhausted_key_indices}` → BFF gọi `PATCH /settings/api-keys` mark exhausted key
